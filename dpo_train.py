...
def dpo_loss(
    ...
    return -1 * torch.log(torch.sigmoid(out))
...
def get_log_probs(model, tokenizer, protein_batch, input_ids):
    ...
    log_probs = log_probs[:, begin_length-1:] # get log_probs only of generated text
...
def train(dataloader, ref_model, train_model, args):
    ...
    optimizer = torch.optim.RMSprop(train_model.parameters(), lr=1e-5)  # default lr=1e-4
...
if __name__ == "__main__":
    ...
    parser.add_argument('--world-size', action='store', dest='world_size', type=int,
                        default=4)
    parser.add_argument('--num-epochs', action='store', dest='n_epochs', type=int,
                        default=1000)
    ...
    parser.add_argument('--restore-from', default='../Trained_model/pocket_generation.pt',
    ...
    ref_model = Token3D(pretrain_path='../Pretrained_model', config=Ada_config)
    train_model = Token3D(pretrain_path='../Pretrained_model', config=Ada_config)
    ...
    protein_matrix = read_data('TODO')
    # contains list of corresponding ligands in smiles + torsion format
    mol_data_win = read_data('TODO')
    mol_data_lose = read_data('TODO')
